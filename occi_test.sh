...
function crud_compute {
  ...
  'text/occi')
    ...
    COMPUTE_LOCATION=`curl -vs -X POST --header "Content-Type: $CONTENT_TYPE" --header "Accept: $ACCEPT" --header "Category: $COM_CATEGORY" --header "X-OCCI-Attribute: $COM_ATTRIBUTE" $COM_LOCATION`
    ;;
  'text/plain')
    BODY="Category: $NET_CATEGORY
X-OCCI-Attribute: $NET_ATTRIBUTE"
...
options=(1 "Create/Read/Update/Delete network" on    # any option can be set to default to "on"
         2 "Create/Read/Update/Delete storage" off
         3 "Create/Read/Update/Delete compute" off
         4 "Create/Read/Update/Delete compute with CDMI storage" off
